...
test('a sealed capture verifies, and its document digest describes what the capture holds', () => {
...
test('the URL and the capture time come from the capture itself', () => {
...
test('a caller may override the URL and the time, and the override is what the claim says', () => {
  ...
  assert.equal(verifyReceipt(sealed.bytes).verified, true);
});

test('an unsigned seal is a receipt that says nobody signs for it', () => {
...
test('a chain head seal reaches L2 on its own', () => {
...
test('sealing the same capture twice produces identical bytes', () => {
  const capture = wacz();
...
test('the claim binds the exact capture it was built from', () => {
  const capture = wacz();
  const sealed = sealFromCapture({ capture, key: key() });
...
test('a capture whose own advertised hash is wrong is refused, not sealed', () => {
...
test('a capture that advertises no WARC is refused, and says what it does advertise', () => {
  const capture = wacz({ advertise: false });
  assert.throws(() => sealFromCapture({ capture, key: key() }), /advertises no WARC record/);
});

test('a capture whose WARC cannot be read is refused rather than sealed with a guess', () => {
...
test('a caller-supplied document is used, and the claim says less because of it', () => {
  const capture = wacz({ warc: utf8('this is not a WARC, whatever it claims') });
...
test('a capture time that is not UTC to the second is refused', () => {
  assert.throws(
...
test('a URL the capture does not hold is refused, with what it does hold', () => {
  assert.throws(
...
test('the WARC is found through the capture’s own datapackage, and read from the container', () => {
...
test('a refusal leaves nothing behind, and never says less than it knows', () => {
