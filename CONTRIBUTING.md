...
1. **A second implementation.** Another language reading
   ...
   and the signature family - 17 of the 21 checks - and agrees with the record on 39 of 55 fixtures. The
   fixtures ship for extending it, so nobody starts by reimplementing anybody's fixture builder:

   ```bash
   node reference/src/vectors.mjs --emit ./kit   # 55 fixtures, the answers, and what to do with them
