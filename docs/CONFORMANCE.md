...
| **A second implementation** | `conformance/verify_claims.py` | **Started, and all 21 checks of section 7.4**: the container, the claim, the claim hash, the capture's document, the signature family, both anchors and the text fingerprint, in Python, written from the specification, with Ed25519 from RFC 8032, a DER/X.509 reader, an RFC 3161 token verifier and a `text-v1` extractor alongside it. It agrees with the record on **45 of 48** fixtures, corroborates 3 refusals, and disagrees on none - a snapshot, to be re-run and re-stated after any vector changes. It also rolls the checks up into `levels`, `verified` and `exit_code` and agrees there too. It is **not** yet offered as a conforming implementation because it compares five of the verdict's nine fields and not the other four (`capture_profile`, `attribution`, `time_bound`, `caveat_count`): `conformance/README.md` says why each is missing, which layers are weaker evidence than the others, and the nine things writing it found. |
