...
| **A second implementation** | `conformance/verify_claims.py` | **Started, and all 21 checks of section 7.4**: the container, the claim, the claim hash, the capture's document, the signature family, both anchors and the text fingerprint, in Python, written from the specification, with Ed25519 from RFC 8032, a DER/X.509 reader, an RFC 3161 token verifier and a `text-v1` extractor alongside it. It agrees with the record on **45 of 48** fixtures, corroborates 3 refusals, and disagrees on none - a snapshot, to be re-run and re-stated after any vector changes. It also compares eight of the verdict's nine fields - `levels`, `verified`, `exit_code`, `capture_profile`, `attribution` and `time_bound` among them. It is **not** yet offered as a conforming implementation because the ninth, `caveat_count`, is not produced, and `conformance/README.md` argues that a count of prose may not belong in section 11 at all; that file also records which layers are weaker evidence than the others, and the nine things writing it found. |
