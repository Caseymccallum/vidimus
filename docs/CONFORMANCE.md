...
| **A second implementation** | `conformance/verify_claims.py` | **Started, and 17 of the 21 checks**: the container, the claim, the claim hash and the signature family in Python, written from the specification, with Ed25519 from RFC 8032 in `conformance/ed25519.py`. It agrees with the record on **42 of 45** fixtures, corroborates 3 refusals, and disagrees on none - a snapshot, to be re-run and re-stated after any vector changes. It is explicitly **not** a conforming implementation: `conformance/README.md` says what is missing and the six things writing it found. |
