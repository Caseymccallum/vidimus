...
`./kit` holds the 55 fixtures, `receipt-vectors.json`, and a README with the three steps: check each fixture
...
| **A second implementation** | `conformance/verify_claims.py` | **Started, and meeting section 11**: all 21 checks of section 7.4, the rollup and `verified` rules of 7.1-7.5, the canonical form byte for byte including its refusals, and every rule-derived field of the recorded verdicts - the container, the claim, the claim hash, the capture's document, the signature family, both anchors and the text fingerprint, in Python, written from the specification, with Ed25519 from RFC 8032, a DER/X.509 reader, an RFC 3161 token verifier and a `text-v1` extractor alongside it. It agrees with the record on **52 of 55** fixtures, corroborates 3 refusals, and disagrees on none - a snapshot, to be re-run and re-stated after any vector changes. It does not produce `caveat_count`, which section 11.1 says is recorded for a reader rather than compared (D-036, and the interest declared there). `conformance/README.md` records which layers are weaker evidence than the others and the nine things writing it found. It verifies the three RSA signature algorithms of RFC 8017 and **not** ECDSA, so a token signed with an ECDSA algorithm is reported `unsupported` rather than guessed at. |
