...
| **A second implementation** | `conformance/verify_claims.py` | **Started, and partial**: the canonical form and the claim hash, in Python, written from the specification. It agrees with the record on **39 of 45** fixtures, corroborates 2 refusals, reaches 4 not at all (named), and disagrees on none - a snapshot, to be re-run and re-stated after any vector changes. It implements 2 of the 21 checks and is explicitly **not** a conforming implementation: `conformance/README.md` says what is missing and what writing it found. |
