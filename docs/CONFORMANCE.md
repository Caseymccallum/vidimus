...
| **A second implementation** | `conformance/verify_claims.py` | **Started, and partial**: the canonical form, the claim hash and the signature family in Python, written from the specification, with Ed25519 from RFC 8032 in `conformance/ed25519.py`. It agrees with the record on **39 of 45** fixtures, corroborates 2 refusals, reaches 4 not at all (named), and disagrees on none - a snapshot, to be re-run and re-stated after any vector changes. It implements 6 of the 21 checks and is explicitly **not** a conforming implementation: `conformance/README.md` says what is missing and what writing it found. |
