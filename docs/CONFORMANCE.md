...
| `npm test` | 105 tests: the canonical form's rules, the container reader, the verifier's invariants, and the vectors. |
...
| **Reading a deflated container in a browser** | `container.readable: unsupported` | The browser's reader is store-only, because a browser's decompression is asynchronous and that reader is not. Closing it means making the reader asynchronous, which is a change to the runtime contract rather than to the format. |
